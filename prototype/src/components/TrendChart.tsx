...
/** 종합 점수 추이 꺾은선 차트 — 손수 그린 SVG, 장식 없음 */
export function TrendChart({ records }: Props) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="trend-chart" role="img" aria-label="점수 추이 차트">
      ...
      {points.map((p, i) => (
        <g key={p.record.id}>
          ...
          {(i % labelEvery === 0 || i === points.length - 1) && (
            <text x={p.x} y={H - 8} textAnchor="middle" className="chart-text num">
