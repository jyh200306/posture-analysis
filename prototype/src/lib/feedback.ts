import type { Feedback, FeedbackArea, ItemKey, ItemScore } from '../types';
...
const ACTIONS: Record<ItemKey, { actions: string[]; minutes: number }> = {
  ...
  balance: {
    actions: ['한 발 서기 좌우 30초씩', '발바닥 전체로 체중을 느끼며 1분 서 있기'],
    minutes: 3,
  },
};

function stateText(key: ItemKey, item: ItemScore): string {
  ...
  switch (key) {
    ...
    case 'spine':
      return `머리–척추–골반 라인이 수직에서 ${value} 기울어진 것으로 측정되었습니다.`;
    case 'neck':
      return `귀가 어깨 중심선보다 약 ${value} 앞으로 나온 것으로 측정되었습니다.`;
    case 'balance':
      return `무게중심이 발목 중심에서 약 ${value} 벗어난 것으로 측정되었습니다.`;
  }
}
...
export function buildFeedback(
  ...
  // 점수 낮은 순으로 정렬해 개선이 필요한 영역만 추린다 (최대 5개)
